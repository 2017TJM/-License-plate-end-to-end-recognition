...
Created bt 2017TJM 2019 12 12

@author: Ma Zhenwei,2017TJM
"""
...
def loss_batch(model, loss_func, data, opt=None):
    ...
    if opt is not None: # 训练阶段只算损失
        ...
        _, ans = torch.max(yb, 2) # 真实标签的下标 torch.Size([1,7])
        _, predicted = torch.max(out_matrix, 2) # 真实标签的下标 torch.Size([1,7])
        compare = (predicted == ans) # torch.Size([1,7])，正确为1，错误为0
        single_correct = compare.sum().item() # 5个字符中预测对几个
        for i in range(batch_size):
            if compare[i].sum().item() == 7:
                whole_correct += 1
        del out_matrix
    loss_item = loss.item()
    del out
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            single_rate = 100 * np.sum(single) / (total_size * 7)
